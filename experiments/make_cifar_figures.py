...
def main():
    ...
    labels = [f"{r['new'].replace('ResNet32-','')} vs {r['old'].replace('ResNet32-','')}"
              for r in rows]
    ...
    ax1.set_title("Exact decomposition (superclass audit)", fontsize=11)
    ax1.legend(fontsize=8.5, loc="lower right", framealpha=0.95)
    ax1.grid(axis="x", alpha=0.25)

    tiers = res["per_tier_at_superclass_phi"]
    names = [t["tier"] for t in tiers if t["new"] == "ResNet32-CB"]
    short = [n.split(" (")[0] for n in names]
    cb = [t["alignment_gain"] for t in tiers if t["new"] == "ResNet32-CB"]
    drw = [t["alignment_gain"] for t in tiers if t["new"] == "ResNet32-DRW"]
