...
CHECKS += [
    ("VG relations audited", "8studies.tex", "227,337",
     vg["comparisons"][0]["n_identified"], 0),
    ("VG test relations", "8studies.tex", "229,605", vgv["n_test"], 0),
    ("CLIP total gain", "8studies.tex", "-0.04655",
     vgv_rows["MLP-VISUAL-S|MLP-SPATIAL-S"]["total_gain"], 1e-5),
    ("CLIP alignment gain", "8studies.tex", "0.00641",
     vgv_rows["MLP-VISUAL-S|MLP-SPATIAL-S"]["reasoning_gain"], 1e-5),
    ("CIFAR CB accuracy", "8studies.tex", "0.2627",
     cif["accuracy"]["CB"], 1e-4),
    ("CIFAR DRW accuracy", "8studies.tex", "0.3874",
...
CHECKS += [
    ("robust CB r10 dR", "8studies.tex", "-0.06903", ms["r10_s0"]["CB cal"]["dR"], 1e-5),
    ("robust CB r50 dR", "8studies.tex", "-0.24409", ms["r50_s0"]["CB cal"]["dR"], 1e-5),
    ("robust DRW r100s1 dR", "8studies.tex", "+0.00748",
     ms["r100_s1"]["DRW cal"]["dR"], 1e-5),
    ("robust DRW r100s2 dR", "8studies.tex", "+0.03124",
     ms["r100_s2"]["DRW cal"]["dR"], 1e-5),
    ("robust LA r50 dR", "8studies.tex", "+0.03768", ms["r50_s0"]["LA cal"]["dR"], 1e-5),
    ("robust CB mean", "8studies.tex", "-0.25108",
     ms["across_seeds_r100"]["CB cal.dR"]["mean"], 1e-5),
    ("robust DRW mean", "8studies.tex", "+0.01953",
     ms["across_seeds_r100"]["DRW cal.dR"]["mean"], 1e-5),
    ("robust r10 CE acc", "8studies.tex", "0.5388",
     ms["r10_s0"]["accuracy"]["CE"], 1e-4),
    ("robust r50 LA acc", "8studies.tex", "0.4550",
...
CHECKS += [
    ("text CE accuracy", "8studies.tex", "0.3532", txt["accuracy"]["CE"], 5e-4),
    ("text CB accuracy", "8studies.tex", "0.3960", txt["accuracy"]["CB"], 5e-4),
    ("text DRW accuracy", "8studies.tex", "0.3776", txt["accuracy"]["DRW"], 5e-4),
    ("text CB superclass dT", "8studies.tex", "0.11388",
     txt_rows["superclass|MLP-CB"]["total_gain"], 1e-5),
    ("text CB superclass dP", "8studies.tex", "0.07071",
     txt_rows["superclass|MLP-CB"]["prior_gain"], 1e-5),
    ("text CB superclass dR", "8studies.tex", "0.04317",
     txt_rows["superclass|MLP-CB"]["reasoning_gain"], 1e-5),
    ("text CB global dR", "8studies.tex", "0.04185",
     txt_rows["global|MLP-CB"]["reasoning_gain"], 1e-5),
    ("text CB tier dR", "8studies.tex", "0.03578",
     txt_rows["tier|MLP-CB"]["reasoning_gain"], 1e-5),
    ("text DRW superclass dT", "8studies.tex", "0.11490",
     txt_rows["superclass|MLP-DRW"]["total_gain"], 1e-5),
    ("text DRW superclass dP", "8studies.tex", "0.11335",
     txt_rows["superclass|MLP-DRW"]["prior_gain"], 1e-5),
    ("text DRW superclass dR", "8studies.tex", "0.00155",
     txt_rows["superclass|MLP-DRW"]["reasoning_gain"], 1e-5),
    ("text DRW global dR", "8studies.tex", "-0.00319",
     txt_rows["global|MLP-DRW"]["reasoning_gain"], 1e-5),
    ("text DRW tier dR", "8studies.tex", "-0.00901",
     txt_rows["tier|MLP-DRW"]["reasoning_gain"], 1e-5),
]
tier_rows = {(r["new"], r["tier"]): r for r in txt["per_tier_at_superclass_phi"]}
CHECKS += [
    ("text CB few-shot dR", "8studies.tex", "0.04991",
     tier_rows[("MLP-CB", "few-shot (<20)")]["alignment_gain"], 1e-5),
    ("text CB medium-shot dR", "8studies.tex", "0.19213",
     tier_rows[("MLP-CB", "medium-shot (20-100)")]["alignment_gain"], 1e-5),
    ("text CB many-shot dR", "8studies.tex", "-0.09227",
     tier_rows[("MLP-CB", "many-shot (>100)")]["alignment_gain"], 1e-5),
    ("text DRW few-shot dR", "8studies.tex", "0.07352",
     tier_rows[("MLP-DRW", "few-shot (<20)")]["alignment_gain"], 1e-5),
    ("text DRW medium-shot dR", "8studies.tex", "0.07171",
     tier_rows[("MLP-DRW", "medium-shot (20-100)")]["alignment_gain"], 1e-5),
    ("text DRW many-shot dR", "8studies.tex", "-0.10753",
