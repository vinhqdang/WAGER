...
We work with Visual Genome \cite{krishna2017visual} in the standard VG150 PredCls
regime, where ground-truth boxes and object labels are supplied and the model has only
to predict one of 50 predicate labels for each ordered object pair. The split provides
532,987 training and 229,605 test relations, drawn from 65,228 and 27,955 images
respectively, and the prior cell throughout is
...
of information. FREQ is simply the training-count distribution conditioned on the class
pair, and FREQ+OVERLAP adds to it a single binary box-overlap indicator. MLP-CLASS
consumes fixed subject and object class embeddings, which makes it a function of $\phi$
alone; MLP-SPATIAL extends those embeddings with 12 normalized box-geometry features
covering relative centre, scale, aspect, intersection, containment and IoU, while
MLP-SPATIAL+ merely increases capacity. Each is trained once on the training split, and
WAGER thereafter consumes nothing but their cached test distributions, scored
quadratically with image-clustered intervals and 499 within-cell cyclic randomizations.
...
Why the prior channel collapses is not mysterious. The $1024$ CLIP dimensions dominate
...
little. The decomposition tells another story entirely: within superclasses the
...
effect but two large ones that happen to cancel. What re-weighting bought was a much
closer fit to the within-superclass frequency distribution, and what it paid for that
was per-instance discrimination, a trade the ten-point accuracy drop confirms
independently. Even the apparent paradox of a proper score that fails to fall with
accuracy has a straightforward explanation. The baseline is badly overconfident,
assigning a mean maximum probability of $0.764$ while being correct on only $0.3662$ of
the test set, so the softer predictions of the re-weighted model recover on calibration
roughly what they surrender on alignment.

DRW is the more representative case, since it is an improvement of the kind a benchmark
submission would actually claim. Its total gain of $+0.06606$ divides into $+0.04206$ of
prior-transported gain and $+0.02400$ of instance alignment (95\% CI
$[+0.01340,+0.03460]$; randomization $p=.002$). Roughly two thirds of what presents
itself as a uniform improvement is therefore recoverable from the class-frequency prior
alone, while the remaining third is a genuine and statistically significant gain in
per-instance discrimination, so deferring the re-weighting evidently does more than
re-rank the classes. Neither an accuracy difference nor an aggregate score can make that
separation, which is the practical argument for reporting the two channels instead of
their sum.

Where the alignment gain sits is visible in Table~\ref{tab:cifar-tier} and
Figure~\ref{fig:cifar}. For DRW it falls where the long-tail literature intends it to,
...
These numbers happen to provide an external check on the theory as well. Computing the
...
$-0.19569$. Theorem~\ref{thm:population} and Proposition~\ref{prop:attenuate} are in
that sense confirmed on real trained models, and not only on the synthetic populations
