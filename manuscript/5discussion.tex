...
another, cross-granularity comparisons mix Proposition~\ref{prop:coarsen}'s between-cell term
with a change of identified subpopulation --- immaterial at VG150's $99.0\%$ coverage, but
worth restricting to a common subsample when coverage is low. The image-clustered interval is
asymptotic, and Theorem~\ref{thm:clt} justifies it in a regime no heavy-tailed benchmark
attains in sample: of $6{,}346$ eligible class-pair cells, $2{,}286$ hold fewer than five
cases, though those carry only $2.7\%$ of identified relations. The reported $94.0\%$
empirical coverage against a nominal $95\%$ therefore remains the operative evidence for the
interval. The randomization test is finite-group exact only under a sharper within-cell
exchangeability null and treats relations rather than images as the randomized units, so it
stays a secondary diagnostic.
...
and the long-tailed studies of Appendix~\ref{app:studies} test that in two other modalities.
The constraint on $\phi$ shows its teeth there: the fine class label cannot serve as a cell,
since it would leave every cell label-homogeneous and drive $\Delta R$ to zero by
construction, producing an artefact that reads exactly like a null result. Elsewhere the
natural choices are easy to name --- a predeclared question template for VQA, an answer
format for other multimodal benchmarks --- though we have not evaluated them.
