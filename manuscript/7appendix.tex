...
large numbers across the (diverging, by (iii)) number of identified examples, and
$\widehat{\Delta R}_c\to_p\Delta R_c$ for each eligible cell by the weak law of large
numbers applied within that cell -- here condition (v), not (iii), is what is used:
(v) guarantees $n_c\to\infty$ almost surely for every cell in the fixed, finite support
of $\phi$, which is what a within-cell WLLN requires; the aggregate condition (iii)
alone bounds only the total identified count, not any individual cell's size, and does
not by itself imply within-cell consistency. So $\widehat\psi_i\to_p\psi_i$ pointwise, and
