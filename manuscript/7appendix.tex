...
Model training and feature extraction for both new experiments ran on a single
cloud GPU runtime (NVIDIA T4, 15\,GB) provisioned and driven from the command line via
the \texttt{google-colab-cli} tool, so that no GPU-hours were spent on the authors'
local machine; the WAGER decomposition itself is pure NumPy and was run locally on the
resulting cached probability arrays, as in Section~\ref{sec:data}. All Python code runs
under a pinned \texttt{conda} environment (Python 3.13).
...
\citep{radford2021learning} runs in half precision with a batch size of $384$ crops and
is never fine-tuned. The MLP head matches MLP-CLASS/MLP-SPATIAL's training recipe: AdamW,
