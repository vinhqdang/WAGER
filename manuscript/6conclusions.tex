...
the point here; reporting both is what we would now recommend to a benchmark able to choose.
Approximate matching would extend the identity to continuous grouping variables, trading
exactness for modelling assumptions. An image-level randomization design preserving several
relation-cell margins at once would strengthen finite-sample inference on structured
benchmarks. And porting multicalibration-style guarantees to pairwise attribution would
address the choice of $\phi$ more convincingly than a declaration rule can. The obstacle to
all of it is the one that limits the method today: benchmarks archive ranked predictions, not
probabilities.
