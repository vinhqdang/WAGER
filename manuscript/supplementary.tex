% Supplementary material for "WAGER: An Exact Decomposition of Model-Pair Score Gains on
% Benchmarks with Strong Label Priors", submitted to Pattern Recognition.
% Submitted as a separate file: the journal's page limit applies to the manuscript, and
% supplementary files appear online exactly as received.
...
% Sections number as S1, S2, ...; floats and equations as S1.1 etc.
...
%% Cross-document references into the manuscript.  The numbers are looked up in
%% main-refs.tex, which experiments/gen_cross_refs.py regenerates from main.aux on every
%% build, so nothing is hard-coded in the prose and the references cannot drift as the
%% manuscript is reordered.  Without the file this document still compiles and prints a
%% bold ?? that build.sh greps for.
\makeatletter
\newcommand{\mainref}[1]{%
  \@ifundefined{mainref@#1}%
    {\textbf{??}\@latex@warning{Cross-document reference `#1' undefined}}%
    {\csname mainref@#1\endcsname}}
\makeatother
\IfFileExists{main-refs.tex}{\input{main-refs.tex}}{}
...
{\large WAGER: An Exact Decomposition of Model-Pair Score Gains\\
on Benchmarks with Strong Label Priors}
\end{center}

\vspace{1em}
\noindent
This document contains the proofs, the influence-function derivation, the implementation and
reproducibility details, and the secondary empirical studies referred to in the manuscript.
Section, equation, table and figure numbers are prefixed with \textsf{S} throughout, and the
manuscript cites them in that form. Numbered results stated in the manuscript are restated
here before being proved, so this document can be read on its own.
...
the main properties Section~\mainref{sec:method} of the manuscript proves in general. Take one cell containing
...
(Theorem~\mainref{thm:population} of the manuscript). Note also that $\Delta P=1/8$ is the largest transported
...
the cell where the old model's did not, and by Supplementary Proposition~S2 the
...
Theorem~\mainref{thm:population} of the manuscript says $\Delta R$ is a within-cell covariance between the
...
Proposition~\mainref{prop:attenuate} of the manuscript shows this holds for every cell with $n_c\ge2$ and every
...
information added; Section~\mainref{sec:simulation} of the manuscript measures that at $-0.488$ in a design where
...
Section~\mainref{sec:vgvisual} of the manuscript exhibits the same structure on real models.
...
\section{Bregman-score identity}\label{app:bregman}
...
For $\psi(q)=\lVert q\rVert_2^2$, $\Delta g_y(x)=2\Delta q_y(x)$ and Eq.~\eqref{eq:bregman-cov-id} is Eq.~(\mainref{eq:cov-id}) of the manuscript. For $\psi(q)=\sum_kq_k\log q_k$, $\Delta g_y(x)=\log q_1(y\mid x)-\log q_0(y\mid x)$, and
\begin{equation}
...
\section{Randomization test}\label{app:randtest}
...
Supplementary Section~S10.4 reports that replacing the class-pair cell with a coarser
...
Supplementary Proposition~S1 in Supplementary Section~S5 states the bound: for a
...
be elicited the same way across benchmarks. Supplementary Section~S5 evaluates both
...
\subsection{Proof of Theorem~\mainref{thm:population} of the manuscript}
...
\]
Subtracting yields the covariance sum in Eq.~(\mainref{eq:population-id}) of the manuscript; the first
...
\operatorname{Cov}(B(X),1)=0$. The remaining terms give Eq.~(\mainref{eq:cov-id}) of the manuscript. If
...
Condition on $C=c$. The proof of Theorem~\mainref{thm:population} of the manuscript above establishes
...
remainder, exactly as in the proof of Theorem~\mainref{thm:population} of the manuscript for the quadratic
case. Substituting,
\begin{align*}
...
Summing the kernel in Eq.~(\mainref{eq:kernel}) of the manuscript over unordered pairs, every observed term
...
applying the population identity of Theorem~\mainref{thm:population} of the manuscript to the left side (at
...
\subsection{Proof of Proposition~\mainref{prop:attenuate} of the manuscript}
...
Eq.~(\mainref{eq:linear}) of the manuscript gives $\sum_{j\ne i}H_i(y_j)=(n_c-1)\hat P_i$. Hence
\[
...
\paragraph{Derivation of Eq.~(\mainref{eq:influence}) of the manuscript.} Fix a cell $c$ and let
...
of Section~\mainref{sec:simulation} of the manuscript, not Eq.~\eqref{eq:cellcount}, that supports the reported
intervals.
...
Eq.~(\mainref{eq:influence}) of the manuscript and has exactly zero sample mean by construction. Its empirical
95\% coverage in a VG-like regime (image-clustered examples, heavy-tailed cell sizes with
a large mass of near-minimum cells) is validated in Section~\mainref{sec:simulation} of the manuscript.
...
\paragraph{Efficient computation.} The implementation does not instantiate all pairs. In addition to Eq.~(\mainref{eq:linear}) of the manuscript,
...
Eq.~(\mainref{eq:influence}) of the manuscript. We use a normal critical value; with 27,955 image clusters,
...
$\sum_y m_{cy}H_i(y)$ in Eq.~(\mainref{eq:linear}) of the manuscript is precomputed; only the assigned label
...
direct numerical checks of Propositions~\mainref{prop:attenuate} of the manuscript and~\ref{prop:coarsen}
...
identity, Corollary~\mainref{cor:dm} of the manuscript's equivalence to the (unclustered) Diebold--Mariano
...
Section~\mainref{sec:data} of the manuscript.
...
Section~\mainref{sec:data} of the manuscript. All Python code runs under a pinned environment (Python 3.13).

\paragraph{Matched-subsample models (Section~\mainref{sec:vgvisual} of the manuscript).} The three compared
...
Section~\mainref{sec:sggaudit} of the manuscript is not exposed in the same way, because its reported
...
Section~\mainref{sec:sggaudit} of the manuscript). What the theorem licenses is the interval's form and its
variance estimator; what licenses reading the reported intervals at finite $N$ is the
coverage simulation of Section~\mainref{sec:simulation} of the manuscript, and Section~\mainref{sec:discussion} of the manuscript
...
throughout Section~\mainref{sec:exp} of the manuscript as asymptotically valid in the idealized regime where
...
(Section~\mainref{sec:simulation} of the manuscript), not this theorem directly, is therefore the evidence that
...
confounding, and Section~\mainref{sec:sggaudit} of the manuscript's audit is unaffected only because its estimate
...
is the coverage simulation of Section~\mainref{sec:simulation} of the manuscript, not this theorem.
...
\section{Composition of the transported gain}\label{app:transported}
...
This section reports the two studies summarized in Section~\mainref{sec:further} of the manuscript and the
granularity, score and cell-size sensitivity analyses referred to there. Each applies the
estimator of Section~\mainref{sec:method} of the manuscript without modification; only the declared grouping
...
predictors are trained here from annotations alone; Section~\mainref{sec:sggaudit} of the manuscript audits
...
of any rank metric, and the real-pixel comparison of Section~\mainref{sec:vgvisual} of the manuscript is
...
the antisymmetric kernel identically zero (Theorem~\mainref{thm:population} of the manuscript), so no
...
Corollary~\mainref{cor:dm} of the manuscript identifies $\widehat{\Delta T}$ and its image-clustered interval in Table
...
(Section~\mainref{sec:sggaudit} of the manuscript) or MLP-SPATIAL beat their baseline for a reason a
...
Sections~\mainref{sec:data}--\mainref{sec:vgvisual} of the manuscript are all instances of one task (predicate
classification on an ordered object pair). Section~\mainref{sec:related-applied} of the manuscript argued
...
Section~\mainref{sec:inference} of the manuscript applies directly. One semantic point matters in this domain:
...
(Section~\mainref{sec:discussion} of the manuscript).
...
(Eq.~(\mainref{eq:cov-id}) of the manuscript), and a monotone recalibration moves it: softening an
...
within-cell prior matching of Section~\mainref{sec:vgvisual} of the manuscript does. It does not:
...
within-group covariance gain directly from the identity of Eq.~(\mainref{eq:cov-id}) of the manuscript, through a
code path that shares nothing with the transport estimator, gives $-0.19530$; multiplying
by the factor $n_c/(n_c-1)$ that Proposition~\mainref{prop:attenuate} of the manuscript predicts, at the
$n_c=500$ of a balanced superclass cell, returns $-0.195693$ against the estimator's
$-0.19569$. The identities of Theorem~\mainref{thm:population} of the manuscript and
Proposition~\mainref{prop:attenuate} of the manuscript are algebraic, so this verifies the implementation
...
To test the breadth Section~\mainref{sec:discussion} of the manuscript claims -- any benchmark supplying two
...
documents are independent units, so the plain H\'ajek interval of Section~\mainref{sec:inference} of the manuscript
...
partition is the conservative default (Section~\mainref{sec:coarsening} of the manuscript): a reader stopping
