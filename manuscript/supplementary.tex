...
the main properties Section~3 of the manuscript proves in general. Take one cell containing
...
(Theorem~Theorem~1 of the manuscript). Note also that $\Delta P=1/8$ is the largest transported
...
Theorem~Theorem~1 of the manuscript says $\Delta R$ is a within-cell covariance between the
...
Proposition~Proposition~2 of the manuscript shows this holds for every cell with $n_c\ge2$ and every
...
information added; Section~4.1 of the manuscript measures that at $-0.488$ in a design where
...
Section~4.4 of the manuscript exhibits the same structure on real models.
...
\section{Bregman-score identity}
...
For $\psi(q)=\lVert q\rVert_2^2$, $\Delta g_y(x)=2\Delta q_y(x)$ and Eq.~\eqref{eq:bregman-cov-id} is Eq.~Eq.~(7) of the manuscript. For $\psi(q)=\sum_kq_k\log q_k$, $\Delta g_y(x)=\log q_1(y\mid x)-\log q_0(y\mid x)$, and
\begin{equation}
...
\subsection{Proof of Theorem~1 of the manuscript}
...
\]
Subtracting yields the covariance sum in Eq.~(7) of the manuscript; the first
...
\operatorname{Cov}(B(X),1)=0$. The remaining terms give Eq.~(8) of the manuscript. If
...
\subsection{Proof of Theorem~2 of the manuscript}
Condition on $C=c$. The proof of Theorem~1 of the manuscript above establishes
...
remainder, exactly as in the proof of Theorem~1 of the manuscript for the quadratic
case. Substituting,
\begin{align*}
...
almost surely. The second term is Eq.~(9) of the manuscript. \qed

\subsection{Proof of Theorem~3 of the manuscript}
Summing the kernel in Eq.~(16) of the manuscript over unordered pairs, every observed term
...
\subsection{Proof of Proposition~1 of the manuscript}
...
applying the population identity of Theorem~1 of the manuscript to the left side (at
$\bar\phi=\bar c$) and to the first term on the right side (at $C=c$, then averaged over
$c$ within $\bar c$) yields Eq.~(11) of the manuscript. \qed

\subsection{Proof of Proposition~2 of the manuscript}
...
Eq.~(18) of the manuscript gives $\sum_{j\ne i}H_i(y_j)=(n_c-1)\hat P_i$. Hence
\[
...
\]
By Eq.~(12) of the manuscript, the left side of each display is exactly
...
\paragraph{Derivation of Eq.~(20) of the manuscript.} Fix a cell $c$ and let
...
of Section~5.1 of the manuscript, not Eq.~\eqref{eq:cellcount}, that supports the reported
intervals.
...
Eq.~(20) of the manuscript and has exactly zero sample mean by construction. Its empirical
95\% coverage in a VG-like regime (image-clustered examples, heavy-tailed cell sizes with
a large mass of near-minimum cells) is validated in Section~5.1 of the manuscript.
...
\paragraph{Efficient computation.} The implementation does not instantiate all pairs. In addition to Eq.~(18) of the manuscript,
...
Eq.~(20) of the manuscript. We use a normal critical value; with 27,955 image clusters,
...
$\sum_y m_{cy}H_i(y)$ in Eq.~(18) of the manuscript is precomputed; only the assigned label
...
direct numerical checks of Propositions~Proposition~2 of the manuscript and~Proposition~1 of the manuscript
(the latter against an explicit discrete population with exact expectations, independent
of the estimator's own code path), Corollary~1 of the manuscript's log-score covariance
identity, Corollary~2 of the manuscript's equivalence to the (unclustered) Diebold--Mariano
...
\paragraph{Full-data VG predictors (Appendices~Section~5.3 of the manuscript--\ref{sec:ablations}).}
...
Appendix~Section~5.3 of the manuscript.
...
\subsection{Compute environment for Appendices~Section~5.4 of the manuscript--\ref{app:lt}}\label{app:compute}
...
Appendix~Section~5.3 of the manuscript. All Python code runs under a pinned environment (Python 3.13).

\paragraph{Matched-subsample models (Appendix~Section~5.4 of the manuscript).} The three compared
...
the subject-only cell, which instantiates Eq.~(12) of the manuscript exactly with
...
Section~5.2 of the manuscript is not exposed in the same way, because its reported
...
is the law-of-total-expectation aggregate of Eq.~(12) of the manuscript's covariance
...
Section~5.2 of the manuscript). What the theorem licenses is the interval's form and its
variance estimator; what licenses reading the reported intervals at finite $N$ is the
coverage simulation of Section~5.1 of the manuscript, and Section~6 of the manuscript
...
throughout Section~5 of the manuscript as asymptotically valid in the idealized regime where
...
(Section~5.1 of the manuscript), not this theorem directly, is therefore the evidence that
...
which is what Proposition~1 of the manuscript governs. It gives no guidance at all between
...
defensible'' cannot detect it, and Proposition~1 of the manuscript is silent because
...
Eq.~(12) of the manuscript, and let
\begin{equation}
...
confounding, and Section~5.2 of the manuscript's audit is unaffected only because its estimate
...
is the coverage simulation of Section~5.1 of the manuscript, not this theorem.
...
This appendix reports the two studies summarized in Section~5 of the manuscript and the
granularity, score and cell-size sensitivity analyses referred to there. Each applies the
estimator of Section~4 of the manuscript without modification; only the declared grouping
...
predictors are trained here from annotations alone; Section~5.2 of the manuscript audits
...
of any rank metric, and the real-pixel comparison of Section~5.4 of the manuscript is
...
the antisymmetric kernel identically zero (Theorem~1 of the manuscript), so no
...
Corollary~2 of the manuscript identifies $\widehat{\Delta T}$ and its image-clustered interval in Table
...
(Section~5.2 of the manuscript) or MLP-SPATIAL beat their baseline for a reason a
...
Appendix~\ref{app:implementation}, support that reading: correcting a model's within-cell
...
to $0.02181$. Proposition~1 of the manuscript identifies why: the coarser cell's estimate
...
Section~4.6 of the manuscript reports that the resulting bias of $0.00742$ is comfortably
...
Sections~Section~5.3 of the manuscript--Section~5.4 of the manuscript are all instances of one task (predicate
classification on an ordered object pair). Section~3.4 of the manuscript argued
...
the global class marginal and so instantiates Proposition~1 of the manuscript in a second
...
Section~4.10 of the manuscript applies directly. One semantic point matters in this domain:
...
(Eq.~(8) of the manuscript), and a monotone recalibration moves it: softening an
...
within-cell prior matching of Section~5.4 of the manuscript does. It does not:
...
within-group covariance gain directly from the identity of Eq.~(8) of the manuscript, through a
code path that shares nothing with the transport estimator, gives $-0.19530$; multiplying
by the factor $n_c/(n_c-1)$ that Proposition~2 of the manuscript predicts, at the
$n_c=500$ of a balanced superclass cell, returns $-0.195693$ against the estimator's
$-0.19569$. The identities of Theorem~1 of the manuscript and
Proposition~2 of the manuscript are algebraic, so this verifies the implementation
...
Coarsening behaves as Proposition~1 of the manuscript predicts. Moving from the 20
...
To test the breadth Section~6 of the manuscript claims -- any benchmark supplying two
...
documents are independent units, so the plain H\'ajek interval of Section~4.10 of the manuscript
...
Coarsening moves both comparisons in the direction Proposition~1 of the manuscript allows
...
partition is the conservative default (Section~4.5 of the manuscript): a reader stopping
