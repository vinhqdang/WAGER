...
(Supplementary Section~S2.1); the finite-sample identity and its $O(NK)$ computation
...
Supplementary Section~S1. A reader who wants only what is needed to use the estimator can
...
drafts of this paper did. Supplementary Proposition~S2 in
Supplementary Section~S9 shows the name is wrong and says what the quantity is
...
Supplementary Section~S3 states the resulting identity and proves it: for a Bregman score
...
Supplementary Section~S2. First, what happens when $\phi$ is made coarser: merging cells
...
Section~S2 replaces it with a bound requiring only a single elicited correlation
$\bar\rho$ between an unrecorded variable's effect on the within-cell gain and its effect on
the within-cell label distribution: the bias is at most $\bar\rho$ times a product of two
...
Supplementary Section~S4 states and proves both, and notes that independence rather than
...
$\widehat{\Delta R}$ under $B$ shifts to obtain a one-sided $p$-value; Supplementary
Section~S5 gives the algorithm. It is exact under the sharp null that labels are exchangeable
...
Supplementary Section~S2 through a H\'ajek-projection expansion. The following supplies
the limit theorem, under conditions that hold whenever the score is bounded and no cluster
dominates the sample.

Supplementary Theorem~S1 in Supplementary Section~S6 states it: if the score is bounded, examples
...
differential between the two models, and Supplementary Theorem~S1 applied to $\widehat{\Delta T}$
