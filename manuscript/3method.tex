...
being merely a sensitivity analysis. Any other Bregman score --- the spherical or
power-family scores catalogued by \citet{gneiting2007proper}, for instance --- inherits an
identity of the same form through its own $\psi$, so Theorem~\ref{thm:bregman} fixes the
scope of the estimator's scoring-rule dependence once rather than leaving each alternative
score to be checked separately. The implemented log score floors each probability at
...
$\widehat{\Delta R}$ under $B$ shifts to obtain a one-sided $p$-value
\begin{equation}
p=\frac{1+\sum_{b=1}^B\ind\{\widehat{\Delta R}^{(b)}\ge\widehat{\Delta R}\}}{B+1},
\label{eq:randp}
\end{equation}
whose plus-one correction counts the observed assignment and so cannot return zero;
Appendix~\ref{app:random} gives the algorithm. It is exact under the sharp null that labels are exchangeable
