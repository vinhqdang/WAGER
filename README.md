...
Submitted to **Journal of Statistical Planning and Inference** (Ms. Ref. No.
JSPI-D-26-00452); currently under major revision. Previously desk-rejected at
*Computational Statistics and Data Analysis*; see [`report/REPORT.md`](report/REPORT.md)
for the full submission history.
...
- **Sensitivity bound.** A Cauchy–Schwarz/Popoviciu bound quantifies how far an
  unrecorded confounder could move the reported alignment gain from what a fully
...
```bash
conda run -n py313 python -m pytest tests -q                        # 16 tests
conda run -n py313 python experiments/verify_manuscript_numbers.py  # 79 quoted values
```
...
tests/                   16 tests, including numerical checks of every proposition/corollary
...
report/REPORT.md         development log across the four revisions
```
